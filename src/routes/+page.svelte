<script lang="ts">
  ...
  const listingsBySeller = data.listings.reduce(
    (acc, listing) => {
    ...
    {} as Record<
      string,
      { user: { display_name: string | null; id: string | null }; listings: typeof data.listings }
    >
  );
  ...
  <div class="mb-8">
    <div class="flex items-center justify-between mb-4">
      <div>
        ...
        <p class="text-sm text-slate-400 mt-1">Real-time updates from the Meeple Cart community</p>
      </div>
    ...
    <div class="space-y-6">
      {#if bulkListings.length > 0}
        ...
        {#each bulkListings as [sellerId, group]}
          <div
            class="bg-gradient-to-r from-emerald-500/10 to-transparent rounded-xl border border-emerald-500/20 p-6"
          >
              ...
              {#each group.listings.slice(0, 6) as listing}
                ...
                >
                  ...
                  <div class="text-xs text-slate-500 mt-1">
                    {listing.listingType === 'sell' && listing.games[0]?.price
                      ? `$${listing.games[0].price}`
                      : listing.listingType}
                  </div>
                </a>
              {/each}
      ...
      {#each data.listings as listing (listing.id)}
        <div class="group relative">
          <div
            class="absolute -left-4 top-4 w-2 h-2 bg-slate-700 rounded-full group-hover:bg-emerald-500 transition"
          ></div>
          ...
          >
            <div class="flex items-start justify-between mb-3">
              <div class="flex items-center gap-3">
                <div
                  class="w-10 h-10 rounded-full bg-gradient-to-br from-emerald-400 to-blue-500 flex items-center justify-center text-white font-semibold"
                >
              ...
              <span
                class="inline-flex items-center rounded-full px-2 py-1 text-xs font-medium {listing.listingType ===
                'sell'
            ...
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-4 text-sm text-slate-500">
                {#if listing.listingType === 'sell' && listing.games[0]?.price}
                  <span class="font-semibold text-emerald-400">${listing.games[0].price}</span>
                {/if}
