...
import type { TradePartyRecord, TradePartySubmissionRecord } from '$lib/types/pocketbase';

export const load: PageLoad = async ({ params }) => {
  const { id } = params;

  try {
    const party = await pb.collection('trade_parties').getOne<TradePartyRecord>(id, {
      expand: 'organizer',
    });

    // Load user's submissions for this party (if logged in)
    const user = get(currentUser);
    let mySubmissions: TradePartySubmissionRecord[] = [];
    let allSubmissions: TradePartySubmissionRecord[] = [];

    if (user) {
      try {
        mySubmissions = await pb
          .collection('trade_party_submissions')
          .getFullList<TradePartySubmissionRecord>({
