<script lang="ts">
  ...
  const groupedListings = groupListingsByCategory(
    data.listings.filter((l: any) => l.type === 'sell' || l.type === 'trade')
  );
...
<svelte:head>
  ...
  <meta name="description" content="Browse board games for sale and trade, organized by category" />
</svelte:head>
        ...
        <div class="mb-6">
          ...
          <div class="space-y-2">
            {#each bggCategories as category}
              <button
                class="w-full text-left rounded-lg border px-3 py-2 text-sm transition {selectedCategory ===
                category.id
      ...
      <div class="mb-6 flex items-center justify-between">
        <div>
          ...
          <p class="text-sm text-slate-400 mt-1">
            {data.listings.filter((l: any) => l.type === 'sell' || l.type === 'trade').length} games
            available
          </p>
        </div>
          ...
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
              ...
              <span class="text-sm font-medium text-emerald-300">
                {cart.size}
                {cart.size === 1 ? 'item' : 'items'} selected
              </span>
            </div>
            <button
        ...
        <div class="space-y-4">
          {#if viewMode === 'grid'}
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {#each groupedListings[selectedCategory] || [] as listing}
                <div
                  class="group relative bg-slate-900/40 rounded-lg border border-slate-800 overflow-hidden hover:border-slate-700 transition"
                >
                  <div
                    class="aspect-square bg-gradient-to-br from-slate-800 to-slate-900 p-4 flex items-center justify-center"
                  >
                  ...
                  <div class="p-4">
                    ...
                    <div class="flex items-center justify-between mt-3">
                      ...
                      <button
                        class="rounded-lg p-2 {cart.has(listing.id)
                          ? 'bg-emerald-500 text-slate-900'
                          : 'bg-slate-800 text-slate-400 hover:text-slate-100'} transition"
          ...
          {:else}
            ...
            <div class="space-y-2">
              {#each groupedListings[selectedCategory] || [] as listing}
                <div
                  class="flex items-center gap-4 bg-slate-900/40 rounded-lg border border-slate-800 p-4 hover:border-slate-700 transition"
                >
                  <div
                    class="w-16 h-16 rounded-lg bg-gradient-to-br from-slate-800 to-slate-900 flex items-center justify-center"
                  >
                  ...
                  <div class="flex-1">
                    ...
                    <p class="text-sm text-slate-500">
                      {listing.condition || 'Good'} · {listing.location}
                    </p>
                  </div>
                  <span class="text-lg font-semibold text-emerald-400">
                    ${listing.price || '??'}
              ...
              <section>
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
                  {#each listings.slice(0, 6) as listing}
                    ...
                    >
                      <div
                        class="aspect-square bg-gradient-to-br from-slate-800 to-slate-900 p-3 flex items-center justify-center"
                      >
                      ...
                      <div class="p-3">
                        ...
                        <div class="flex items-center justify-between mt-2">
                          ...
                          <button
                            class="rounded p-1 {cart.has(listing.id)
                              ? 'bg-emerald-500 text-slate-900'
                              : 'bg-slate-800 text-slate-400 hover:text-slate-100'} transition"
                            on:click={() => toggleCartItem(listing.id)}
                            aria-label="Add to cart"
