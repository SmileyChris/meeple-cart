...
<main class="mx-auto max-w-5xl px-4 py-8">
  ...
  <div class="mb-8">
    ...
    <p class="text-slate-400">
      Help fellow gamers find the games they're looking for. These listings will expire after 30 days of inactivity.
    </p>
      ...
      <div class="bg-slate-900/60 rounded-lg border border-slate-800 p-3">
        <div class="text-xl font-semibold text-emerald-400">
          {data.listings.filter(l => {
            const days = Math.floor((new Date().getTime() - new Date(l.created).getTime()) / (1000 * 86400));
            return days < 7;
          }).length}
        </div>
  ...
  <!-- Create Want Button -->
  <div class="mb-6 rounded-lg bg-gradient-to-r from-purple-500/10 to-transparent border border-purple-500/20 p-6">
    <div class="flex items-center justify-between">
      <div>
        <h3 class="font-semibold text-purple-300 mb-1">Looking for a specific game?</h3>
        <p class="text-sm text-slate-400">Create a wanted listing and let the community know what you're searching for.</p>
      </div>
      <a
  ...
  <div class="space-y-4">
    {#each sortedWantedListings as listing}
      {@const daysSinceCreated = Math.floor((new Date().getTime() - new Date(listing.created).getTime()) / (1000 * 86400))}
      ...
      <div class="bg-slate-900/40 rounded-xl border border-slate-800 p-5 hover:border-slate-700 transition">
        <div class="flex items-start justify-between mb-3">
          ...
          <div class="flex items-center gap-2">
            <!-- Freshness indicator -->
            <span class="inline-flex items-center rounded-full px-2 py-1 text-xs font-medium {freshnessClass}">
              {daysSinceCreated < 7 ? 'Fresh' :
               daysSinceCreated < 14 ? 'Active' :
               daysSinceCreated < 30 ? 'Expiring Soon' : 'Expired'}
            </span>
            
            <!-- Priority/Urgency badge -->
            {#if listing.urgent}
              <span class="inline-flex items-center rounded-full px-2 py-1 text-xs font-medium bg-red-500/10 text-red-400">
                Urgent
              </span>
            {/if}
          </div>
        ...
        {#if listing.max_price}
          <div class="inline-flex items-center gap-2 rounded-lg bg-slate-800/50 px-3 py-1.5 text-sm mb-3">
            <svg class="w-4 h-4 text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <span class="text-slate-300">Willing to pay up to <strong class="text-emerald-400">${listing.max_price}</strong></span>
          </div>
        {/if}
        ...
        {#if daysSinceCreated > 20}
          <div class="mt-3 rounded-lg bg-orange-500/10 border border-orange-500/20 p-3">
            <p class="text-xs text-orange-300">
              ⚠️ This listing will expire in {30 - daysSinceCreated} days unless the owner adds a comment or updates it.
            </p>
          </div>
        {/if}
      </div>
  ...
  {#if data.pagination.totalPages > 1}
    <div class="mt-8 text-center">
      <button class="rounded-lg border border-slate-700 px-6 py-2 text-sm text-slate-300 hover:border-purple-500 hover:text-purple-300 transition">
