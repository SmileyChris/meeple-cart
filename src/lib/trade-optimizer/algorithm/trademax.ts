...
enum PriorityType {
	NO_PRIORITIES = 1,
	LINEAR_PRIORITIES = 2,
	TRIANGLE_PRIORITIES = 3,
	SQUARE_PRIORITIES = 4,
	SCALED_PRIORITIES = 5,  // no longer supported!!
	EXPLICIT_PRIORITIES = 6,
}

type NameMap = { [key: string]: Vertex }

type Metric = {
	name: string,
	calculate: (cycles: Vertex[][]) => number
}
...
class MetricSumSquares {
	...
	calculate(cycles: Vertex[][]) {
		...
		this.counts = this.counts.sort(function (a: number, b: number) {
			return b - a;
		});
	...
	toString() {
		return `[ ${this.sumOfSquares} : ${this.counts.map((c) => c.toString()).join(' ')} ]`;
	}
} // class MetricSumSquares
...
var metricmap: { [key: string]: Metric } = {};
...
class Entry {
	...
	constructor(vertex: Vertex, cost: number, heap: Heap) {
		this.vertex = vertex;
		this.cost = cost;
		this.heap = heap;
		this.child = null;
	...
	decreaseCost(toCost: number) {
		if (!this.heap.root) throw new Error('Heap is empty')
		this.cost = toCost;
		if (this == this.heap.root || !this.prev || this.cost >= this.prev.cost) return;
...
class Heap {
	...
	extractMin() {
		...
		if (list) {
			while (list?.sibling) {
				let nextList = null;
				while (list && list.sibling) {
					let a: Entry = list;
					let b: Entry = list.sibling;
	...
	insert(vertex: Vertex, cost: number) {
...
class Vertex {
	...
	edgeMap: { [vertexName: string]: Edge }
	edgeList: Edge[]
	edges: Edge[]
	...
	constructor(name: string, user: string | null, isDummy: boolean, type: VertexType) {
		this.name = name;
		this.user = user;
...
class Edge {
	...
	constructor(receiver: Vertex, sender: Vertex, cost: number) {
...
class Graph {
	tm: TradeMaximizer
	hasBeenFullyShrunk: boolean
	sinkFrom: Vertex | null
	sinkCost: number | null
	receiverList: Vertex[] | null
	senderList: Vertex[] | null
	receivers: Vertex[]
	senders: Vertex[]
	orphans: Vertex[]
	frozen: boolean
	...
	addVertex(name: string, user: string | null, isDummy: boolean) {
		if (!this.receiverList || !this.senderList) throw new Error('Graph is frozen');
		let receiver = new Vertex(name, user, isDummy, VertexType.RECEIVER);

		this.receiverList.push(receiver);
		this.nameMap[name] = receiver;

		let sender = new Vertex(name, user, isDummy, VertexType.SENDER);
	...
	addEdge(receiver: Vertex, sender: Vertex, cost: number) {
		let edge = new Edge(receiver, sender, cost);
		receiver.edgeMap[sender.name] = edge;
		sender.edgeMap[receiver.name] = edge;
	...
	removeImpossibleEdgesAndOrphans() {
		...
		for (let v of this.receivers) if (v.mark != this.timestamp) this.visitReceivers(v);
	...
	removeOrphans() {
		...
		for (let v of this.receivers) {
			if (v.edges.length > 1 || v.edges[0].sender != v.twin) newreceivers.push(v);
		...
		for (let v of this.senders) {
			if (v.edges.length > 1 || v.edges[0].receiver != v.twin) newsenders.push(v);
		}
	...
	findBestMatches() {
		...
		for (let v of this.senders) {
			...
			if (v.dirty) {
				...
				for (let edge of v.edges) v.minimumInCost = Math.min(edge.cost, v.minimumInCost);
				v.dirty = false;
			}
	...
	findCycles() {
		...
		for (let vertex of this.receivers) {
			if (vertex.mark == this.timestamp || vertex.match == vertex.twin) continue;
	...
	shrink(level: number, verbose: boolean) {
		...
		if (verbose) this.tm.outputln('Shrink 1 time = ' + (Date.now() - startTime) + 'ms');

		if (level > 1) {
			this.findForbiddenEdgesAndShrink(verbose);
			this.removeImpossibleEdgesAndOrphans();
			this.reportStats('Shrink 2 (SCC)', verbose);
			if (verbose) this.tm.outputln('Shrink 2 time = ' + (Date.now() - startTime) + 'ms');
	...
	scaleUpEdgeCosts(factor: number) {
		for (let v of this.senders) {
			v.dirty = true;
	...
	scaleDownEdgeCosts(factor: number) {
		for (let v of this.senders) {
			v.dirty = true;
	...
	findRequiredEdgesAndShrink(verbose: boolean) {
		...
		for (let v of this.receivers) this.removeEdges(v, EdgeStatus.FORBIDDEN);
		for (let v of this.senders) this.removeEdges(v, EdgeStatus.FORBIDDEN);
...
*/
	...
	findForbiddenEdgesAndShrink(verbose: boolean) {
		...
		for (let v of this.receivers) this.removeEdges(v, EdgeStatus.UNKNOWN);
		for (let v of this.senders) this.removeEdges(v, EdgeStatus.UNKNOWN);
	...
	removeEdges(v: Vertex, statusToRemove: EdgeStatus) {
		let newedges = [];
	...
	reportStatsOrDot(name: string, verbose: boolean) {
		if (verbose) this.reportStats(name, verbose);
		else this.tm.output('.');
	}

	reportStats(name: string, verbose: boolean) {
		if (!verbose) return;

		let histogram = [];
		let edgeCount = 0;

		histogram[EdgeStatus.REQUIRED] = 0;
		histogram[EdgeStatus.OPTIONAL] = 0;
		histogram[EdgeStatus.UNKNOWN] = 0;
		...
		this.tm.outputln(
			...
			edgeCount +
			' REQUIRED=' +
			histogram[EdgeStatus.REQUIRED] +
			' OPTIONAL=' +
			histogram[EdgeStatus.OPTIONAL] +
			' UNKNOWN=' +
			histogram[EdgeStatus.UNKNOWN]
		);
	}
...
export class TradeMaximizer {
	iterations: number;
	priorityScheme: PriorityType;
	smallStep: number;
	bigStep: number;
	nonTradeCost: number;
	shrinkLevel: number;
	shrinkVerbose: boolean;
	officialNames: { [key: string]: string | true };
	usedNames: string[];
	graph: Graph | null;
	errors: string[];
	options: string[];
	...
	requireUsernames: boolean;
	showErrors: boolean;
	showRepeats: boolean;
	showLoops: boolean;
	showSummary: boolean;
	showNonTrades: boolean;
	showStats: boolean;
	showMissing: boolean;
	sortByItem: boolean;
	...
	constructor() {
		this.iterations = 1;
		this.priorityScheme = PriorityType.NO_PRIORITIES
		this.smallStep = 1;
		this.bigStep = 9;
	...
	readLine() {
		return this.infunc ? this.infunc() : null;
	...
	fatalError(message: string, lineNumber?: number) {
		if (this.fatal) {
			this.fatal(message);
		} else {
			//outputln(s);
	...
	run(input: InputFunc, out: OutputFunc, progress?: ProgressFunc, fatal?: FatalFunc) {
		this.infunc = input;
		this.outfunc = out;
...
*/
		...
		if (this.iterations > 1 && this.progress)
		...
		if (this.iterations > 1) {
			...
			for (let i = 1; i <= this.iterations - 1; i++) {
				if (this.progress) this.progress(i, this.iterations, bestMetricStr, tradeCount);
...
*/
				...
				if (foundBetter) {
					...
					tradeCount = 0;
					for (let cycle of bestCycles) tradeCount += cycle.length;
				} else if (this.verbose) this.outputln('# ' + newmetricsStr + ' (iteration ' + i + ')');
			}
		...
		if (this.showElapsedTime) this.outputln('Elapsed time = ' + (stopTime - startTime) + 'ms');
	}

	sumOfSquares(cycles: Vertex[][]) {
		let sum = 0;
		...
		for (let lineNumber = 1; ; lineNumber++) {
			...
			if (line.match('#!.*')) {
				...
				for (let option of optionslist) {
					if (option == 'CASE-SENSITIVE') this.caseSensitive = true;
					...
					else if (option == 'LINEAR-PRIORITIES') this.priorityScheme = PriorityType.LINEAR_PRIORITIES;
					else if (option == 'TRIANGLE-PRIORITIES') this.priorityScheme = PriorityType.TRIANGLE_PRIORITIES;
					else if (option == 'SQUARE-PRIORITIES') this.priorityScheme = PriorityType.SQUARE_PRIORITIES;
					else if (option == 'SCALED-PRIORITIES') {
						this.priorityScheme = PriorityType.SCALED_PRIORITIES;
						//this.fatalError("SCALED-PRIORITIES no longer supported!",lineNumber);
					} else if (option == 'EXPLICIT-PRIORITIES') this.priorityScheme = PriorityType.EXPLICIT_PRIORITIES;
						...
						for (let m of metrics) {
							if (metricmap[m] == null) this.fatalError('unknown METRIC=' + m, lineNumber);
			...
			if (readingOfficialNames) {
				if (line.charAt(0) == ':') this.fatalError('Line cannot begin with colon', lineNumber);
				if (line.charAt(0) == '%')
					this.fatalError('Cannot give official names for dummy items', lineNumber);
			...
			if (line.indexOf('(') == -1 && this.requireUsernames)
				this.fatalError('Missing username with REQUIRE-USERNAMES selected', lineNumber);
			if (line.charAt(0) == '(') {
				if (line.lastIndexOf('(') > 0)
					this.fatalError("Cannot have more than one '(' per line", lineNumber);
				let close = line.indexOf(')');
				if (close == -1) this.fatalError("Missing ')' in username", lineNumber);
				if (close == line.length - 1)
					this.fatalError('Username cannot appear on a line by itself', lineNumber);
				if (line.lastIndexOf(')') > close)
					this.fatalError("Cannot have more than one ')' per line", lineNumber);
				if (close == 1) this.fatalError('Cannot have empty parentheses', lineNumber);

				// temporarily replace spaces in username with #'s
				if (line.indexOf(' ') < close)
					line = line.substring(0, close + 1).replace(/ /g, '#') + ' ' + line.substring(close + 1);
			}
	...
	buildGraph(wantLists: string[][]) {
		if (!this.graph) throw new Error('Graph not initialized');

		let unknowns: { [key: string]: number } = {};
		...
		for (let i = 0; i < wantLists.length; i++) {
			...
			if (this.officialNames != null && !this.officialNames[name] && name.charAt(0) != '%') {
				...
				if (this.officialNames != null && this.officialNames[name]) this.usedNames.push(name);

				if (!isDummy) this.width = Math.max(this.width, this.show(vertex).length);
			}
		}
		...
		for (let list of wantLists) {
			...
			for (let i = 1; i < list.length; i++) {
				...
				if (toName.indexOf('=') != -1) {
					if (this.priorityScheme != PriorityType.EXPLICIT_PRIORITIES) {
						this.errors.push(
				...
				if (toVertex == fromVertex.twin)
					...
					if (this.showRepeats)
						this.errors.push(
							'**** Item ' + toName + ' is repeated in want list for ' + fromName + '.'
						);
				} else if (
					!toVertex.isDummy &&
					...
					switch (this.priorityScheme) {
						case PriorityType.LINEAR_PRIORITIES:
							cost = rank;
							break;
						case PriorityType.TRIANGLE_PRIORITIES:
							cost = (rank * (rank + 1)) / 2;
							break;
						case PriorityType.SQUARE_PRIORITIES:
							cost = rank * rank;
							break;
						case PriorityType.SCALED_PRIORITIES:
							cost = rank;
							break; // assign later
						case PriorityType.EXPLICIT_PRIORITIES:
							cost = rank;
							break;
					}
		...
		for (let entry in unknowns) {
			...
			this.errors.push(
				'*** Unknown item ' + entry + ' (' + unknowns[entry] + ' occurrence' + plural + ')'
			);
		}
	} // buildGraph
	...
	displayMatches(cycles: Vertex[][]) {
		...
		if (this.showNonTrades) {
			for (let v of this.graph.receivers) {
				if (v.match == v.twin && !v.isDummy)
					summary.push(this.show(v) + '             does not trade');
			}

			for (let v of this.graph.orphans) {
				if (!v.isDummy) summary.push(this.pad(this.show(v)) + '             does not trade');
			}
		}
		...
		if (this.showStats) {
			this.output(
				'Num trades   = ' + numTrades + ' of ' + (this.ITEMS - this.DUMMY_ITEMS) + ' items'
			);
